import KoAccsmntListClient from "./KoAccsmntListClient";
...
async function getItechList(queryInstance: SearchCrawlingCommand) {
  const headersInstance = headers();
  const hostUrl = makeHostUrl(headersInstance);
  const res = await serverSideFetch(
    hostUrl +
      `/api/common/crawling/getItechList?${makeUrlQuery(queryInstance)}`,
  );
...
export default async function KoAccsmntListServer() {
  ...
  const { data }: ApiResponse<TablePageResponse<ItechResponse[]>> =
    await getItechList(queryInstance);
