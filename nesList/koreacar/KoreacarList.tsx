...
interface KoreacarListProps {
  menuName: string;
  data: TablePageResponse<KamaResponse[]>;
  queryInstance: SearchBoardCrawlingCommand;
}

export default function KoreacarList({
  menuName,
  data,
  queryInstance,
}: KoreacarListProps) {
  ...
  const headers: TableHeader[] = useMemo(() => {
    return [
      ...
      },
      {
        ...
        width: "80%",
        align: "left",
        accessFn: (item: KamaResponse) => {
          return (
            <Link
              href={`https://www.kama.or.kr/${item.titleHref}`}
              target="_blank"
            >
              {item.title}
            </Link>
      ...
      <div className={style.page_wrap}>
                  ...
                  router.push(
                    `/${
                      pathname?.includes("/virtual")
                        ? "virtual"
                        : "reusebattery"
                    }/nt/nesList/koreacar?${makeUrlQuery({
                      ...searchInstance,
                      page: 0,
                    })}`
                  );
        ...
        <TableTop countName="총" count={data ? data.totalElements : 0}>
          <Button
            ...
            //style={{ width: "155px" }}
            onClick={() => {
              const excel_data = data.content.map((item: KamaResponse) => {
                return {
                  no: String(item.no),
                  title: item.title,
              ...
              xlsxData["!cols"] = [{ wpx: 100 }, { wpx: 336 }, { wpx: 210 }];
