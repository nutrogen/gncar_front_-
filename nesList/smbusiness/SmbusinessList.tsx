...
interface SmbusinessListProps {
  menuName: string;
  data: TablePageResponse<MssResponse[]>;
  queryInstance: SearchBoardCrawlingCommand;
}

export default function SmbusinessList({
  menuName,
  data,
  queryInstance,
}: SmbusinessListProps) {
  ...
  const headers: TableHeader[] = useMemo(() => {
    return [
      ...
      },
      {
        ...
        width: "35%",
        align: "left",
        accessFn: (item: MssResponse) => {
          return (
            <Link
              href={`https://www.mss.go.kr/site/smba/ex/bbs/${item.titleHref}`}
              target="_blank"
            >
              {item.title}
            </Link>
      ...
      <div className={style.page_wrap}>
                  ...
                  router.push(
                    `/${
                      pathname?.includes("/virtual")
                        ? "virtual"
                        : "reusebattery"
                    }/nt/nesList/smbusiness?${makeUrlQuery({
                      ...searchInstance,
                      page: 0,
                    })}`
                  );
        ...
        <TableTop countName="총" count={data ? data.totalElements : 0}>
          <Button
            ...
            //style={{ width: "155px" }}
            onClick={() => {
              const excel_data = data.content.map((item: MssResponse) => {
                return {
                  no: String(item.no),
                  title: item.title,
